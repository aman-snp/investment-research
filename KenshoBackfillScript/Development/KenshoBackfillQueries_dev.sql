...
CREATE TABLE [dbo].[CodexWorkingSet_tbl](
	[newKeyFileCollectionDetail] [int] IDENTITY(-2093732092 ,1) NOT NULL, ---this has to change for production 
...
Go
Insert into CodexStepLastId_tbl values (2, -2093732092)
Insert into CodexStepLastId_tbl values (3, -2093732092)
Insert into CodexStepLastId_tbl values (4, -2093732092)
...
AS (
	...
	FROM InternalUseOnly_New..CodexIntermediate cix WITH (NOLOCK)
	WHERE cix.updoperation < 2
		AND SweeperProcessingPriority = 202
		AND SweeperProcessingCompleted IS NULL
	)
Insert into  Codex_00.dbo.CodexWorkingSet_tbl (KeyCodexIntermediate, KeyFileVersion, KeyFileCollectionDetail, CodexDocubotVersion)
...
CREATE NONCLUSTERED INDEX cx_temp ON Codex_00..CodexWorkingSet_tbl (KeyFileVersion, CodexDocubotVersion)
CREATE NONCLUSTERED INDEX cx_temp1 ON Codex_00..CodexWorkingSet_tbl (KeyFileCollectionDetail)
...
BEGIN
	...
	FROM Codex_00..CodexWorkingSet_tbl cxws  WITH (NOLOCK)
	Where cxws.newKeyFileCollectionDetail >@KFCDID
	
	INSERT INTO Codex_00..FileCollectionDetailWO (
	...
	IF NOT EXISTS (Select Top 1 1 from Codex_00..CodexWorkingSet_tbl WITH(NOLOCK) Where newKeyFileCollectionDetail > @MaxId)
	BEGIN
...
BEGIN
	...
	Declare @S3URIPath nvarchar(MAX)='scdev://codex/';
	...
	SELECT Top (@BatchCount) 
			...
			into TempCodexWorkingSet
		FROM Codex_00..CodexWorkingSet_tbl cxwt  WITH (NOLOCK)
		Join InternalUseOnly_new..CodexIntermediate cx WITH (NOLOCK) ON cx.KeyFileVersion = cxwt.KeyFileVersion
		Where cxwt.newKeyFileCollectionDetail >@KFCDID
	
	--Display
	INSERT INTO Codex_00..FileVersionWO (
		KeyFileCollectionDetail
		,KeyFileVersion
		,FileURI
		,Hashvalue
...
Declare @KFCDID int = (Select Top 1 lastRecordId from CodexStepLastId_tbl WITH(NOLOCK) Where StepId =4)
	...
	VALUES (
		...
		BEGIN
			Drop table TempCodexConversionWorkingSet
		END
		SELECT Top (@BatchCount) cxwt.newKeyFileCollectionDetail,
		...
		FROM Codex_00..CodexWorkingSet_tbl cxwt  WITH (NOLOCK)
		inner join InternalUseOnly_new..CodexIntermediate cx on cx.KeyFileVersion= cxwt.KeyFileVersion and cxwt.CodexDocubotVersion = cx.CodexDocubotVersion
		Where cxwt.newKeyFileCollectionDetail >@KFCDID

	Insert Into Codex_00..fileconversioninputoutput_tbl
			(
...
BEGIN
	...
	Insert into Codex_00..FileConversionInputWO(KeyFileConversionInput,KeyFileConversion,KeyFileVersion,KeyConversionProcessVersion,UpdOperation, Upddate)
	select 
	newid() as KeyFileConversionInput, KeyFileConversionCodexAnnot , InputKeyFileVersion ,b.KeyConversionProcessVersion , 0 , GETDATE()
	from Codex_00..fileconversioninputoutput_tbl a (nolock) inner join publish2..ConversionProcessVersion b (nolock) on a.CodexDocubotVersion= b.ConversionProcessVersion
...
VALUES (
	...
	Insert into Codex_00..FileConversionOutputWO(KeyFileConversionOutput,KeyFileConversion,KeyFileVersion,UpdOperation, Upddate)
	select newid() as KeyFileConversionOutput, KeyFileConversionCodexAnnot, OutPutKeyFileVersionCodexAnnot,0,GETDATE() from Codex_00..fileconversioninputoutput_tbl (nolock)
	Where OutPutKeyFileVersionCodexAnnot is not null
	
	Insert into Codex_00..FileConversionOutputWO(KeyFileConversionOutput,KeyFileConversion,KeyFileVersion,UpdOperation, Upddate)
	select newid() as KeyFileConversionOutput, KeyFileConversionCodexText, OutPutKeyFileVersionCodexText,0,GETDATE() from Codex_00..fileconversioninputoutput_tbl  (nolock)
	Where OutPutKeyFileVersionCodexText is not null
	
	Insert into Codex_00..FileConversionOutputWO(KeyFileConversionOutput,KeyFileConversion,KeyFileVersion,UpdOperation, Upddate)
	select newid() as KeyFileConversionOutput, KeyFileConversionCodexCoord, OutPutKeyFileVersionCodexCoord,0,GETDATE() from Codex_00..fileconversioninputoutput_tbl  (nolock) 
	Where OutPutKeyFileVersionCodexCoord is not null
	
	Insert into Codex_00..FileConversionOutputWO(KeyFileConversionOutput,KeyFileConversion,KeyFileVersion,UpdOperation, Upddate)
	select newid() as KeyFileConversionOutput, KeyFileConversionCodexPDFInfo, OutPutKeyFileVersionCodexPDFInfo,0,GETDATE() from Codex_00..fileconversioninputoutput_tbl (nolock)
	Where OutPutKeyFileVersionCodexPDFInfo is not null
	
	Insert into Codex_00..FileConversionOutputWO(KeyFileConversionOutput,KeyFileConversion,KeyFileVersion,UpdOperation, Upddate)
	select newid() as KeyFileConversionOutput, KeyFileConversionCodexDisplay, OutPutKeyFileVersionCodexDisplay,0,GETDATE() from Codex_00..fileconversioninputoutput_tbl  (nolock)
...
VALUES (
	...
	Insert into Codex_00..fileconversionWO(KeyFileConversion,KeyConversionProcessVersion,KeyFileConversionStatus, UpdOperation, Upddate)
	select KeyFileConversion,KeyConversionProcessVersion,3,0, GETDATE() from Codex_00..FileconversionInputWO (nolock)
...
from Codex_00..FileConversionInputWO a with (nolock) Where not exists 
(
Select 1 from Codex_00..FileConversionOutputWO b with(nolock) Where a.KeyFileConversion =b.KeyFileConversion
)
	
	Create clustered index cx_temp on #fileconversionwithoutOutputversion_tbl(KeyFileConversion)
	
	Delete a from  Codex_00..FileConversionInputWO a Where Exists (Select 1 from #fileconversionwithoutOutputversion_tbl b Where a.KeyFileConversion=b.KeyFileConversion)
	Delete a from  Codex_00..FileConversionOutputWO a Where Exists (Select 1 from #fileconversionwithoutOutputversion_tbl b Where a.KeyFileConversion=b.KeyFileConversion)
	Delete a from  Codex_00..fileconversionWO a Where Exists (Select 1 from #fileconversionwithoutOutputversion_tbl b Where a.KeyFileConversion=b.KeyFileConversion)
...
begin 
	...
	select  top (10000) cxwt.KeyFileCollectionDetail, cxwt.keyfileversion 
		into #FC_batch_tbl
		from Codex_00..CodexWorkingSet_tbl cxwt  WITH (NOLOCK)
				inner Join InternalUseOnly_new..CodexIntermediate cx WITH (NOLOCK)
		...
		set SweeperProcessingCompleted = getdate()
			FROM #FC_batch_tbl cxwt  WITH (NOLOCK)
				inner Join InternalUseOnly_new..CodexIntermediate cx  ON cx.KeyFileVersion = cxwt.KeyFileVersion
				where SweeperProcessingCompleted is null 
...
End 
	...
	If not exists (select count(*) from Codex_00..CodexWorkingSet_tbl cxwt  WITH (NOLOCK)
				inner Join InternalUseOnly_NEW..CodexIntermediate cx WITH (NOLOCK)
					ON cx.KeyFileVersion = cxwt.KeyFileVersion
				where SweeperProcessingCompleted is null)
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Populate File Collection Detail', 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'EXEC Populate_fileconversionWO_tbl_prc 10000', 
		@database_name=N'Codex_00', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_update_job @job_id = @jobId, @start_step_id = 1
...
--Codex_00 restored on all the subscribers  
--		-> only be staging 
...
--DocsFileCollectionDetail -> 15 million
--DocsFileVersion -> 75 million
--DocsFileConversion -> 75 million
--DocsFileConversionInput -> 75 million
--DocsFileConversionInput -> 75 million
----10 days
--->